  ...
  <div>
    ...
    <table class="centered-table">
      ...
      <tr v-for="member in teamMembers" :key="member.nick_name">
        <td class="table-cell">{{ member.NickName }}</td>
...
export default {
  ...
  mounted() {
    ...
    console.log("team是", team);
    axios.get(`/api/teamDetails?team=${team}`)
        .then(response => {
          this.teamDesc = response.data.data.TeamDesc;
        })
        .catch(error => {
          console.error('发生错误:', error);
        });

    axios.get(`/api/teamsMember?team=${team}`)
        .then(response => {
